...
if [ $FLAG = "t" ]; then
    ...
    cat $OPTARG | xargs -I {} github-endpoints -d {} -o urls.github-unsort 1>/dev/null
fi
...
######################## Spidering Actively with gospider ########################
...
######################## JS links Enumeration ########################
tput setaf 42; echo -n "[+] JS links enum passive & active: "
